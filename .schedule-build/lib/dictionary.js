...
exports.getEntryClue = getEntryClue;
...
function inferFamiliarity(word) {
    if (word.length === 5) {
        return 9;
    }
    if (word.length >= 6) {
...
function buildTags(word, override) {
...
exports.DICTIONARY = RAW_DICTIONARY.map((entry) => {
    const override = DICTIONARY_OVERRIDES[entry.word];
    const clues = Array.from(new Set([...(override?.clues ?? []), entry.clue]));
    return {
        word: entry.word,
        clue: clues[0],
        clues,
...
function getEntryClue(entry, seed) {
